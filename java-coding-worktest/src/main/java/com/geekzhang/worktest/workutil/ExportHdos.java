 ...
 */
@Slf4j
public class ExportHdos {
    public static void main(String[] args) throws IOException {

//        System.out.println( 9/10 +"%");

        InputStream resourceAsStream0 = Thread.currentThread().getContextClassLoader().getResourceAsStream("order.json");
        JSONArray store = JSON.parseObject(resourceAsStream0 , JSONArray.class);


        List<StoreInfo> storeInfos = JSON.parseArray(JSON.toJSONString(store), StoreInfo.class);


        InputStream resourceAsStream1 = Thread.currentThread().getContextClassLoader().getResourceAsStream("brand.json");
        JSONArray brand = JSON.parseObject(resourceAsStream1 , JSONArray.class);


        List<Brand> brandInfos = JSON.parseArray(JSON.toJSONString(brand), Brand.class);


        Map<Integer,Brand> brandMap = brandInfos.stream().collect(Collectors.toMap(Brand::getBrandId, Function.identity()));


        InputStream resourceAsStream2 = Thread.currentThread().getContextClassLoader().getResourceAsStream("province.json");
        JSONArray province = JSON.parseObject(resourceAsStream2 , JSONArray.class);


        List<Province> provinceInfos = JSON.parseArray(JSON.toJSONString(province), Province.class);

        Map<String,Province> provinceMap = provinceInfos.stream().collect(Collectors.toMap(Province::getValue, Function.identity()));
        ...
        storeInfos.forEach(item->{
            ...
            try {
                ...
                String preUrl1 =
                        "https://wwww.cn/work/v2/group/statistics/integral";
